...
//https://blog.csdn.net/jiary5201314/article/details/41213561
...
void print_packet_tcp(const u_char* packet, int length){
	printf("Source port:%d\nDestination port:%d\n", (packet[0] * 0x100 + packet[1]), (packet[2] * 0x100 + packet[3]));
	printf("Sequence number(raw):%02x%02x02x02x\n", packet[4], packet[5], packet[6], packet[7]);
	printf("Acknowledgment number(raw):%02x%02x02x02x\n", packet[8], packet[9], packet[10], packet[11]);	
}
...
void print_packet_ip(const u_char* packet, int length){
	printf("Version:%d\nHeader length:%d\n", (packet[0] & 0xF0)>>4, packet[0] & 0x0F);
	printf("Ip packet length:%d\n", (packet[2] * 0x100 + packet[3]));
	printf("Flag:MF=%d DF=%d\n", (packet[6] & 0x20) >> 5, (packet[6] & 0x40)>>6);
	printf("Fragment offset: %d\n", (packet[6] & 0x1F) * 0x100 + packet[7]);
	printf("TTL:%d\n", packet[8]);
	printf("Protocol:%d\n", packet[9]);
...
//打印出以太网MAC帧的头部信息
void print_packet_mac(const u_char* packet, const struct pcap_pkthdr *pkthdr){
	int pos = 0;
...
void processPacket( u_char *arg, const struct pcap_pkthdr *pkthdr, const u_char *packet )
{
	...
	print_packet_mac(packet, pkthdr);
	if(is_quiet)
		return;
	printf( "Payload:\n" );
